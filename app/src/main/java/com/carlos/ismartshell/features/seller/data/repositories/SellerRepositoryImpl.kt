...
class SellerRepositoryImpl(private val apiService: ApiService) : SellerRepository {

    override suspend fun getStores(): List<SellerStore> {
        return apiService.getStores().map {
            SellerStore(it.id, it.name, it.description, it.address)
        }
    }

    override suspend fun createStore(request: CreateStoreRequest): SellerStore {
        return apiService.createStore(request).toDomain()
    }
    ...
    override suspend fun updateStore(id: Int, request: UpdateStoreRequest): SellerStore {
        return apiService.updateStore(id, request).toDomain()
    }

    override suspend fun deleteStore(id: Int) {
